...
from .math_renderer import add_math_row
...
def set_paragraph_line_break_controls(
    shape,
    *,
    ...
    for paragraph in shape.text_frame.paragraphs:
        ...
        p_pr.set("eaLnBrk", "1" if east_asian_line_break else "0")
        p_pr.set("latinLnBrk", "1" if latin_line_break else "0")
        p_pr.set("hangingPunct", "1" if hanging_punctuation else "0")


def word_wrap_text(text: str, max_chars: int) -> str:
    if max_chars <= 0:
        return text
...
def is_multiline_item(item: PracticeItem) -> bool:
    ...
    if source_line_count >= 3:
        return True
    return source_line_count >= 2 and (expr.count("=") >= 2 or len(expr) >= 68)
...
def add_blank_if_needed(group, text: str, x: float, y: float, design: dict[str, Any] | None) -> None:
    blank_count = text.count("[ ]") + text.count("__") + text.count("□")
...
def add_item_group(
    ...
    *,
    ...
    explicit_display_lines = len([line for line in item.display_lines if line.strip()]) >= 2
    formula_size = style_value(design, "problem_slide.item.formula_font_size", 21)
    if multiline and not explicit_display_lines:
        formula_size = style_value(design, "problem_slide.worked_item.formula_font_size", min(18, formula_size))
        formula_h = max(formula_h, style_value(design, "problem_slide.worked_item.formula_h", 0.42))
    ...
    line_gap = formula_h if explicit_display_lines else (
        style_value(design, "problem_slide.worked_item.line_gap", 0.40) if multiline else formula_h
    )
...
def add_problem_slide(prs: Presentation, block: PracticeBlock, items: list[PracticeItem], continuation: int, design: dict[str, Any] | None) -> None:
    ...
    if style_value(design, "problem_slide.prompt.word_wrap.enabled", True):
        prompt = word_wrap_text(prompt, int(style_value(design, "problem_slide.prompt.word_wrap.line_char_limit", 42)))
    prompt_shape = add_text(
        slide.shapes,
        style_value(design, "problem_slide.prompt.x", 0.224),
        prompt_y,
        style_value(design, "problem_slide.prompt.w", 9.559),
        prompt_h,
        prompt,
    ...
    set_paragraph_line_break_controls(
        prompt_shape,
        east_asian_line_break=bool(style_value(design, "problem_slide.prompt.word_wrap.east_asian_line_break", False)),
        latin_line_break=bool(style_value(design, "problem_slide.prompt.word_wrap.latin_line_break", False)),
        hanging_punctuation=bool(style_value(design, "problem_slide.prompt.word_wrap.hanging_punctuation", True)),
    )
